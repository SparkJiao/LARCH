...
class QueryEncoderExpandVGG(nn.Module):
            ...
            for image_id in all_images:
                ...
                if image_id != 0 and image_path.is_file():
                    try:
                        ...
                        image = QueryEncoderExpandVGG.transform(image)
                    except OSError:
                        image = QueryEncoderExpandVGG.EMPTY_IMAGE
                else:
                    image = QueryEncoderExpandVGG.EMPTY_IMAGE
                image_embeddings.append(image)
            image_embeddings = torch.stack(image_embeddings).to(DEVICE)
            image_embeddings = self.vgg(image_embeddings).view(num_images, -1)
